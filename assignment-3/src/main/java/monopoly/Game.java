...
public class Game {
  ...
  private void gameModeSelection() {
    char selection = ui.promptForPlayModeSelection();
    if (selection == 'y' || selection == 'Y') {
      do {
        String computerPlayerName = ui.promptForNewComputerPlayerName();
        if (computerPlayerName != null && computerPlayerName.length() > 0) {
          Player computer = new ComputerPlayer(board.getStartTile(), computerPlayerName, ui);
      ...
      System.out.println("No computer player will be added!");
    } else {
